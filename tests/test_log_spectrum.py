...
from deeprhythm.audio_proc.log_spectrum import (
    ...
    compute_log_spectrum_from_clips,
...
def test_log_spectrum_concatenates_clip_frames():
    magnitude = torch.rand(4, 1025, 345)
    spectrum = compute_log_spectrum_from_clips(magnitude)

    assert spectrum.shape == (81, 1 + 30 * 22050 // 512)
    assert float(spectrum.amin()) == pytest.approx(0)
    assert float(spectrum.amax()) == pytest.approx(1)

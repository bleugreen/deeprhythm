...
from conftest import make_click_track, make_silence, write_wav
...
def test_predict_returns_float_in_range(predictor, tmp_path):
    ...
    audio = make_click_track(120, duration=16)
    path = write_wav(audio, 22050, tmp_path)
...
def test_predict_from_audio_matches_predict(predictor, tmp_path):
    ...
    sr = 22050
    audio = make_click_track(120, duration=16, sr=sr)
    path = write_wav(audio, sr, tmp_path)
...
def test_predict_per_frame_count(predictor, tmp_path):
    ...
    sr = 22050
    audio = make_click_track(120, duration=24, sr=sr)
    path = write_wav(audio, sr, tmp_path)
...
def test_predict_click_track(predictor, tmp_path, target_bpm):
    ...
    audio = make_click_track(target_bpm, duration=24)
    path = write_wav(audio, 22050, tmp_path)
    result = predictor.predict(path)
...
def test_predict_silence(predictor, tmp_path):
    ...
    audio = make_silence(duration=16)
    path = write_wav(audio, 22050, tmp_path)
    bpm, conf = predictor.predict(path, include_confidence=True)
    assert 30 <= bpm <= 286
...
def test_predict_exactly_8_seconds(predictor, tmp_path):
    ...
    sr = 22050
    audio = make_click_track(120, duration=8, sr=sr)
...
def test_predict_single_vs_multi_clip(predictor, tmp_path):
    ...
    sr = 22050
    audio_short = make_click_track(120, duration=8, sr=sr)[:sr * 8]
    audio_long = make_click_track(120, duration=16, sr=sr)

    path_short = write_wav(audio_short, sr, tmp_path, name="short.wav")
    path_long = write_wav(audio_long, sr, tmp_path, name="long.wav")
